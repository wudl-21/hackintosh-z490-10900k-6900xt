...
> [!TIP]
> The guide above used gfxutil and IORegistryExplorer to view the hardware info, but Hackintool is capable of handling it alone. You can find the info needed (eg. path to GFX0@0) under the `PCIe` tab.
...
> [!WARNING]
> The guide above provided multiple approaches but among which it is recommended to implement the DMAR modification using MaciASL. The latest method is to add a kext called `AppleIGC.kext`, but this may sometimes cause the boot loader being unable to detect the disk drive.
